...
// Tasks ids are init in the main file so we can use them when printing debug info.
...
void _displayFirstRow() {
  ...
  if (t1 == SENSOR_ERROR) {
    ...
    // lcd.print(t1, 1);  // To use only 1 decimal.
    lcd.print(t1);
...
void _displaySecondRow() {
  ...
  if (sht85Data[0] == SENSOR_ERROR) {
    ...
    lcd.print(sht85Data[0]);
    // lcd.print((String)F("\xDF") + "C ");
    lcd.print("\xDF"
              "C ");
    // lcd.print(sht85Data[1], 0);  // To use 0 decimals.
