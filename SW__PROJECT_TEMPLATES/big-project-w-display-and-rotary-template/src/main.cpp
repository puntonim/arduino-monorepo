...
/* TODO
 - mostra un timer sul display che parte da 30 sec on boot
 - poi nella vers successiva, il timer è a 0 on boot, ma parte quando
    giro il rotary
 - aggiorna il timer solo quando il display è acceso
 - quando giri il rotary aggiungi o sottrai secondi
 - aggiungi un errore di test, giusto per vedere il led
 - quando arriva a zero accendi la led matrix
 - (? non formattare per bene il tempo ma metti una nota che l'ho fatto in
 thermostat project)
 - rimuovi segreti dal file al contrario
 - rimuovi led matrix se non la uso
 - riscrivi la docstring in main.cpp
*/
