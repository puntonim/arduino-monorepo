...
void ErrorManager::addDs18b20SensorError() {
  ...
  pubSub.publish(new pubsub_utils::ErrorStatusChangeEvent(true, _errorMessageListForDisplay));
  _handleErrors();
}
...
void ErrorManager::removeDs18b20SensorError() {
  ...
  if (!areThereErrors()) pubSub.publish(new pubsub_utils::ErrorStatusChangeEvent(false));
  _handleErrors();
}
