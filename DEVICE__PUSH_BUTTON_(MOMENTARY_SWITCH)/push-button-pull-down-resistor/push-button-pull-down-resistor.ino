 ...
 *
 * Note: the button is connected with a pull-down resistor, which is not the smartest
 *  choice as Arduino has integrated pull-up resistors on every PIN (so you don't 
 *  even need to add a pull-up resistor but just tell Arduino to use it).
 *  HIGH-LOW state are inverted compared to the pull-down resistor.
 *
 * Docs:
